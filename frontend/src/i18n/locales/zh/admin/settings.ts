export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        ticketPageTitle: '292 打票',
        ticketPageDescription: '票据续期、代理池与自动补充，集中管理。',
        ticketHeroTitle: '自动续票，持续就绪',
        ticketHeroDescription: '按账户和模型维护票据，在到期前自动重新获取。',
        ticketEnabled: '已开启',
        ticketPaused: '已暂停',
        ticketLoading: '读取状态中',
        ticketSwitch: '启用自动打票',
        ticketStock: '池内代理',
        ticketStockHint: '按顺序轮换使用',
        ticketRemoved: '累计移除',
        ticketRemovedHint: '获取失败后自动清理',
        ticketRenewal: '提前续票',
        ticketMinutes: '{minutes} 分钟',
        ticketRenewalHint: '票据剩余此时间时开始获取',
        ticketRefill: '自动补充',
        ticketRefillHint: '池空时每批请求 2,000 条',
        ticketSupply: '补充代理',
        ticketSupplyDescription: '通过接口提取，或直接导入已有代理。',
        ticketApiTab: 'API 提取',
        ticketManualTab: '批量导入',
        ticketRules: '当前运行规则',
        ticketRenewRule: '剩余 {minutes} 分钟开始续票',
        ticketRenewRuleHint: '新票获取成功后，自动替换旧票。',
        ticketRemoveRule: '失败 {count} 次自动移除',
        ticketRemoveRuleHint: '按实际获取结果处理，成功则保留。',
        ticketRefillRule: '代理用完，自动补充',
        ticketRefillDisabledHint: '当前未开启，可在代理补充中启用并保存。',
        ticketRefillRuleHint: '每批请求 2,000 条；补充失败后等待 60 秒再试。',
        ticketBackground: '任务在服务器后台运行，关闭页面也会继续。',
        ticketUpdated: '更新于 {time} · 每 15 秒刷新',
        ticketLoadError: '状态加载失败，请点击刷新重试。',
        ticketSaveError: '保存失败，请稍后重试。',
        ticketSaved: '设置已保存',
        ticketConfigure: '配置接口',
        ticketCollapse: '收起配置',
        ticketNotConfigured: '待配置',
        ticketAdvanced: '单代理设置',
        ticketAdvancedHint: '未使用代理池时生效。留空保存会保留已配置的代理。',
        ticketAdvancedPlaceholder: '填写完整代理连接',
        ticketSaveProxy: '保存代理',
        ticketManage: '打开打票管理',
        ...
        codexTicketHarvestProxyDesc:
          '仅在门票功能开启时用于打票，保存后后续探测会使用新代理，无需重启。日常业务仍走账号自己的住宅代理。填写完整代理 URL（http 或 socks5h，含用户名和密码）。可在下方批量导入代理；导入后优先使用代理池，池空时暂停打票。单个代理输入留空并保存表示不改已保存的值。',
        ...
        poolTitle: "批量导入 IP / 代理池",
        poolRefresh: "刷新状态",
        poolRule: "开启 292 打票后轮流使用池内代理。只要 1 次未获取到有效票据就自动删除该代理，获取成功则保留。",
        poolStatus: "当前 {count} 条代理 · 已自动删除 {removed} 条",
        poolEmpty: "代理池已空，暂停打票并等待补充。开启自动补充和 292 打票后会在后台获取新代理，也可手动导入。",
        poolInput: "代理列表（每行一条）",
        poolFormats: "支持 IP:端口、IP:端口:用户名:密码，以及 HTTP、HTTPS、SOCKS5、SOCKS5H 代理 URL；单次最多 2000 条。",
        poolImport: "批量导入 IP",
        poolImporting: "正在导入…",
        poolFile: "选择 TXT 文件",
        poolImmediate: "追加导入并自动去重，立即保存生效。",
        poolLoadError: "无法加载代理池状态，请点击刷新重试。",
        poolFileTooLarge: "文件不能超过 2 MB。",
        poolLimit: "请填写 1 至 2000 条代理，每行一条。",
        poolSuccess: "已新增 {added} 条，跳过重复 {duplicates} 条，当前共 {count} 条。",
        poolImportError: "导入失败，原有代理保持不变。",
        apiTitle: "通过 OnesProxy API 获取",
        apiDescription: "填写 API 信息后即可获取代理并自动导入，也可使用现成的提取链接。",
        apiType: "接口方式",
        apiGenerate: "账号 API（Userid / Token）",
        apiExtract: "已有提取 API 链接",
        apiProxyID: "动态代理编号（proxy_id）",
        apiSecretSaved: "已保存，留空保留；填写新值可更换",
        apiTokenPlaceholder: "在 OnesProxy 支持中心 → API集成中查看",
        apiCountry: "国家代码（留空随机）",
        apiRoute: "线路代码（与套餐一致，可留空）",
        apiMode: "代理模式",
        apiSticky: "粘性会话",
        apiRotate: "每次连接轮转",
        apiSession: "粘性时间（分钟，须在套餐范围内）",
        apiURL: "完整提取链接（含 key）",
        apiCount: "单次提取数量",
        apiLimitNote: "默认请求 2000 条，以供应商实际返回数量为准；代理连接条数不等于不同出口 IP 的数量。",
        apiAutoRefill: "代理池用完后自动补充",
        apiAutoRefillNote: "保存后生效：开启 292 打票且代理池为空时，后台自动请求 2000 条并导入，无需保持页面打开。最多每分钟提取一次，失败后等待 60 秒再重试。",
        apiFetching: "正在获取并导入…",
        apiFetch: "获取并导入",
        apiSave: "仅保存配置",
        apiConfigured: "API 已配置",
        apiLoadError: "无法读取 API 配置，请刷新重试。",
        apiMissingCredentials: "请填写 Userid、Token 和动态代理编号。",
        apiMissingURL: "请填写完整的 OnesProxy 提取 API 链接。",
        apiSaved: "API 配置已保存。",
        apiSuccess: "请求 {requested} 条，实际返回 {received} 条；新增 {added} 条，重复 {duplicates} 条，池内共 {count} 条。",
        apiError: "API 获取失败，原有代理保持不变。",
