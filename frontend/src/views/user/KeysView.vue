  ...
  <AppLayout>
        ...
        <div>
          <label class="input-label flex items-center gap-1.5">
            ...
            <span class="group relative inline-flex">
              ...
              <span
                class="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 w-56 -translate-x-1/2 rounded-lg bg-gray-900 px-2.5 py-1.5 text-xs font-normal text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100 dark:bg-dark-700"
              >
                {{ t('keys.groupLockedHint') }}
              </span>
            </span>
          ...
          <div
            v-else
            class="flex min-h-[42px] items-center rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 dark:border-dark-600 dark:bg-dark-700/60"
          >
            <GroupBadge
              v-if="selectedKeyGroup"
              :name="selectedKeyGroup.name"
              :platform="selectedKeyGroup.platform"
              :subscription-type="selectedKeyGroup.subscription_type"
              :rate-multiplier="selectedKeyGroup.rate_multiplier"
              :user-rate-multiplier="userGroupRates[selectedKeyGroup.id]"
              :peak-rate-enabled="selectedKeyGroup.peak_rate_enabled"
              :peak-start="selectedKeyGroup.peak_start"
              :peak-end="selectedKeyGroup.peak_end"
              :peak-rate-multiplier="selectedKeyGroup.peak_rate_multiplier"
            />
    ...
    <BaseDialog :show="!!createdKeyForImport" :title="t('keys.keyCreatedSuccess')" @close="createdKeyForImport = null">
      <div class="flex gap-3">
        ...
        <button class="btn btn-secondary" @click="createdKeyForImport = null">{{ t('keys.quickImport.later') }}</button>
      </div>
    </BaseDialog>
...
import TablePageLayout from '@/components/layout/TablePageLayout.vue'
	...
	import QuickImportModal from '@/components/keys/QuickImportModal.vue'
...
const handleSubmit = async () => {
  // Group is selected only during creation; existing keys keep their original group.
  ...
  try {
    if (showEditModal.value && selectedKey.value) {
      ...
      createdKeyForImport.value = createdKey
